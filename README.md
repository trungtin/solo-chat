...
This is a chat for people who only want to talk to themselves. Unfortunately, the app isn't done yet. There are some components, but they haven't been connected in any way.

You are free to implement the specification below however you see fit. Add any state, props, modules, patterns or components you'd like.

**Specification**

* When you press the Send button, any text in the text input field should be appended to the chat area.
* If the user inputs text beginning with `/`, instead of appending the string, the following should happen:
  * `/time`: the current time should be appended to the chat area.
  * `/starwars <string>`: the application should use [SWAPI](https://swapi.co/documentation) to search for characters matching the given string and append the first result (if any) to the chat area.
  * `/goodbye`: the Send button should be disabled.
  * For any other slash command, a text informing the user that no such command exists should be appended to the chat area.
* The data does not need to be persisted between page reloads.
